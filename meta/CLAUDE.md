# Caelestia fork glue — meta bundle

This directory is the "meta bundle" that keeps the three personal caelestia
forks in sync and provides the multi-root VS Code workspace. It's installed
automatically by `../scripts/install.sh`; this doc describes the wiring so
it's easy to audit or adjust.
...
Each of these clones has `origin` → user fork and `upstream` → caelestia-dots
upstream. `scripts/install.sh` sets that up on first clone.

## Syncing forks with upstream

```sh
yay -Syu                   # 1. system + AUR deps (hyprland, quickshell-git, foot, fonts, ...)
./meta/update.sh           # 2. fork sync + plugin rebuild
```

Order matters: update `quickshell-git` first (pacman/yay), then rebuild the
native plugin against it (the script does the rebuild for you).

Script options:

```sh
./meta/update.sh shell           # just one repo
./meta/update.sh --force-stash   # auto-stash dirty trees before syncing
```
...
onto the new `main` (aborting cleanly on conflict). Use `--force-stash` if
the tree is dirty.
...
`update.sh` rebuilds+reinstalls automatically after syncing the shell repo.
Session env wiring lives in `~/.config/environment.d/caelestia-plugin.conf`
(loaded by the systemd user manager, inherited by UWSM). A re-login (not
just a reload) is required after changing it.
...
```sh
git checkout main && git pull upstream main    # or run update.sh
